...
namespace XPT.Core.Scripting.LoxScript.VirtualMachine {
    internal partial class Gears { // all state variables and ctor
        ...
        private GearsFunctionInfo[] _FunctionInfos; // cached info for all top-level functions.
        ...
        /// <summary>
        /// Returns information about all top-level functions, including their names and parameter counts.
        /// This should be called after the script has been initialized (after running to populate globals).
        /// </summary>
        internal IEnumerable<GearsFunctionInfo> GetTopLevelFunctions() {
            if (_FunctionInfos == null) {
                List<GearsFunctionInfo> infos = new List<GearsFunctionInfo>();
                foreach (string name in Globals.AllKeys) {
                    ...
                    infos.Add(new GearsFunctionInfo(name, function.Arity));
                }
                _FunctionInfos = infos.ToArray();
            }
