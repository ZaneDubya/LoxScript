...
namespace XPT.Core.Scripting.Rules {
    ...
    internal static class RuleSystem {
        ...
        /// </summary>
        internal static void LoadRule(object key, Rule rule) {
        ...
        /// When the handling object is disposed, you must also call UnloadRules.
        /// </summary>
        internal static void LoadRules(object key, RuleCollection rules) {
        ...
        /// </summary>
        internal static void UnloadRules(object key) {
            _RuleCollections.Remove(key);
        }

        internal static void Invoke(string triggerName, RuleInvocationContext context, Action<object> resultHandler, params object[] args) {
            foreach (RuleCollection rules in _RuleCollections.Values) {
                foreach (Rule rule in rules.GetMatching(triggerName, context)) {
                    object returned = rule.Invoke(args);
                    resultHandler?.Invoke(returned);
                }
            }
        }
    }
